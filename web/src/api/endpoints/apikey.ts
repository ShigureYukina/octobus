    ...
    return useQuery({
        ...
        select: (data): APIKeyStatsResponseFormatted => ({
            stats: {
                ...
                cache_hit_token: formatCount(data.stats.cache_hit_token || 0),
                cache_hit_rate: data.stats.input_token > 0 ? `${(((data.stats.cache_hit_token || 0) / data.stats.input_token) * 100).toFixed(1)}%` : '0%',
            },
            info: data.info,
        }),
