...
import { Clock, Cpu, Zap, AlertCircle, ArrowDownToLine, ArrowUpFromLine, DollarSign, ArrowRight, ArrowDown, Send, MessageSquare, Loader2, RotateCw, ChevronDown, ChevronUp, Pin, Database, Activity } from 'lucide-react';
...
function formatCacheHitRate(cacheHitTokens: number | undefined, inputTokens: number): string {
    if (!cacheHitTokens || cacheHitTokens <= 0 || inputTokens <= 0) return '-';
    return `${((cacheHitTokens / inputTokens) * 100).toFixed(1)}%`;
}
                ...
                >
                    <div className={cn("p-4 grid grid-cols-[auto_1fr] gap-4", hasError ? "items-start" : "items-center")}>
                        ...
                        <div className="min-w-0 flex flex-col gap-3">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-x-4 gap-y-2 text-xs tabular-nums text-muted-foreground">
